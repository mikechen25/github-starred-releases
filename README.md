[README.md](https://github.com/user-attachments/files/31870813/README.md)
...
| 文件 | 作用 |# GitHub Starred Releases RSS
...
- **无需任何 PAT / Secret**：运行时使用 GitHub Actions 自动注入的 `GITHUB_TOKEN`（作用域仅限本仓库、自动轮换、无存储）。整个仓库零 Secrets。
- **去重**：Feed 每次由当前 API 快照确定性重建，按 Release 数字 ID 保证不重复；Release Notes 被作者编辑后会自动同步。
- **频率**：每小时（cron `23 * * * *` UTC）+ 可手动触发（Actions → 该 workflow → Run workflow）。另有每月 1 个空提交保活（`keepalive.yml`，每月 1 日 03:17 UTC 无条件 push 一次），防止 GitHub 因 60 天无活动停掉定时任务。
- **失败保护**：抓取 Star 列表或任意仓库 Releases 遇网络/限流/临时错误时，本轮构建直接失败、**不部署**，线上保留上一次成功的 `index.xml`，不会被残缺 Feed 覆盖。
- **Feed 地址（公开）**：`https://mikechen25.github.io/github-starred-releases/index.xml`
  - GitHub Pages 免费版为公开站点：任何拿到该 URL 的人都能访问。
  - 你当前的 Star 列表本身在 GitHub 上也是公开的，本 Feed 不额外暴露你的账号私有信息。
...
| 文件 | 作用 |
|---|---|
| `.github/workflows/feed.yml` | 每小时构建并部署 Feed 到 Pages（权限：contents: read + pages: write + id-token） |
| `.github/workflows/keepalive.yml` | 每月 1 个空提交保活（唯一拥有 contents: write 的 workflow） |
