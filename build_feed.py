...
Why this satisfies the rules:
  ...
  * Pre-releases DO appear (alpha/beta/rc etc.), becau#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
build_feed.py — Build an Atom feed of GitHub Releases for every repo starred by a user.
...
Failure protection (fail-closed):
  * If the starred-repo list cannot be fetched, or looks empty while that is not
    explicitly allowed, the run FAILS and nothing is deployed.
  * If ANY repo's releases endpoint fails with a network / rate-limit / GitHub
    temporary error, the run FAILS and nothing is deployed.
  * The previous successful deployment therefore stays live instead of being
    overwritten by a broken or partial feed. Only HTTP 404 for a single repo is
    tolerated (repo deleted / made private in the meantime) - that is not a
    temporary error, and the repo will simply drop out of the next run's star list.
"""
import html
import json
import os
...
ALLOW_EMPTY_STARS = os.environ.get("GH_ALLOW_EMPTY", "0") == "1"
...
def get_all_pages(url):
    ...
    Any HTTP error here aborts the whole run (fail-closed): an unreachable star
    list must never produce an empty or partial feed.
    """
    result = []
    page = 1
    while True:
        ...
        try:
            ...
            raise RuntimeError(
                "starred-list request failed (HTTP {c}) for {u} page {p}".format(c=err.code, u=url, p=page)
            ) from err
        except urllib.error.URLError as err:
            raise RuntimeError("network error while fetching {u}: {r}".format(u=url, r=err.reason)) from err
        if not batch:
            break
        result.extend(batch)
...
def main():
    ...
    if not stars:
        if ALLOW_EMPTY_STARS:
            print("starred list is empty; GH_ALLOW_EMPTY=1 -> continuing with empty feed")
        else:
            raise RuntimeError(
                "starred-repo list came back empty. Refusing to deploy an empty feed "
                "(set GH_ALLOW_EMPTY=1 only if that is truly expected)."
            )
    ...
    for repo in stars:
        ...
        try:
            ...
            if err.code == 404:
                # Repo deleted / renamed / became private since the star list was read.
                # Permanent condition, not a temporary error: skip it, it will drop
                # out of the star list on its own next run.
                skipped += 1
                print("skip {f}: HTTP 404 (repo not accessible)".format(f=full), file=sys.stderr)
                continue
            raise RuntimeError(
                "releases request failed for {f} (HTTP {c}) - aborting this build".format(f=full, c=err.code)
            ) from err
        except urllib.error.URLError as err:
            raise RuntimeError(
                "network error while fetching releases for {f}: {r}".format(f=full, r=err.reason)
            ) from err
        for rel in releases:
            if rel.get("draft"):
...
if __name__ == "__main__":
    try:
        main()
    except Exception as exc:  # fail-closed: any anomaly -> non-zero exit, no deployment
        print("FATAL: {e}".format(e=exc), file=sys.stderr)
        print("Build aborted - the previously deployed index.xml stays untouched.", file=sys.stderr)
        sys.exit(1)
se they are published releases.
  * No state is kept between runs: the feed is rebuilt deterministically from the
    current API snapshot every run, so an entry never duplicates, newly starred
    repos appear on the next run, and unstarred repos disappear on the next run.
"""
